...
try {
    ...
    if ($endpoint === '') {
        ers_external_json(422, [
            ...
            'hint' => 'Send group5_endpoint in the query string or set GROUP5_ANALYTICS_ENDPOINT in .env.',
            'payload' => $payload,
        ]);
    }
    ...
    $result = ers_analytics_get_json($endpoint, $payload, $input);
    ...
    error_log('send_analytics_get.php failed: ' . $e->getMessage());
...
function ers_analytics_get_json(string $endpoint, array $payload, array $input): array
{
    ...
    $url = $endpoint . $separator . http_build_query($payload);
...
function ers_analytics_insert_sync_log(PDO $pdo, string $status, string $endpoint, array $payload, int $entityId): int
{
    ...
    $stmt = $pdo->prepare(
        "INSERT INTO api_sync_logs
         ...
         VALUES
            ('outgoing', 'Group 5', 'send_analytics_get', 'response_time_analytics', ?, ?, ?, NOW(), NOW())"
    );
    $stmt->execute([
        $entityId,
        $status,
